    ...
    for _, problem := range problems {
        _, err := s.db.Exec(`
            ...
            ON CONFLICT (frontend_id) DO UPDATE SET
                ...
                url = EXCLUDED.url
        `, problem.Title, problem.Difficulty, problem.AcceptanceRate, problem.FrontendID, problem.IsPremium, problem.URL)

        if err != nil {
            return fmt.Errorf("error inserting problem %s: %v", problem.Title, err)
        }
    }
    return nil
}
